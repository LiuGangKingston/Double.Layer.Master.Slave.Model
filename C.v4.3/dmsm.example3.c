...
!
!    This is test example 3 of square root summation
...
int  Number_of_Results_in_Thread;
int *Jobs_in_Thread;
double *Results_in_Thread;
#pragma omp threadprivate(Number_of_Results_in_Thread, Jobs_in_Thread, Results_in_Thread)
...
{ int i,j;
  ...
  Number_of_Results_in_Thread=0;

  omp_set_num_threads(Total_Number_Of_Threads_Per_Process);
#pragma omp parallel default(shared)
  {
   if((Jobs_in_Thread=(int *) malloc(Total_Num_Of_Jobs*sizeof(int)))==NULL)
              {printf( "Error on memory allocation in Data_Initialize().\n") ;
               MPI_Finalize(); exit(0);}
   if((Results_in_Thread=(double *) malloc(Total_Num_Of_Jobs*sizeof(double)))==NULL)
              {printf( "Error on memory allocation in Data_Initialize().\n") ;
               MPI_Finalize(); exit(0);}
   }
}
...
void Do_The_Job(int my_job)
{
   ...
   Jobs_in_Thread[Number_of_Results_in_Thread]=my_job;
   Results_in_Thread[Number_of_Results_in_Thread]=r;
   Number_of_Results_in_Thread++;
}
...
{  int i,tag=73,additional;
   ...
   if(from==DMSM_Get_Master())
     {for(i=0; i<Number_of_Results_in_Thread; i++)
         {Final_Total_Jobs_In_Master[   Total_Of_Final_Results_In_Master]=   Jobs_in_Thread[i];
          Final_Total_Results_In_Master[Total_Of_Final_Results_In_Master]=Results_in_Thread[i];
      ...
      if(my_rank==DMSM_Get_Master())
        {
         ...
         {MPI_Send(&Number_of_Results_in_Thread,1,MPI_INT,DMSM_Get_Master(),tag,MPI_COMM_WORLD);
          if(Number_of_Results_in_Thread > 0)
            {MPI_Send(   Jobs_in_Thread,Number_of_Results_in_Thread,MPI_INT,   DMSM_Get_Master(),tag,MPI_COMM_WORLD);
             MPI_Send(Results_in_Thread,Number_of_Results_in_Thread,MPI_DOUBLE,DMSM_Get_Master(),tag,MPI_COMM_WORLD);
             Number_of_Results_in_Thread=0;}
          }
     }
...
{  FILE * fout;
   ...
   if (My_MPI_Rank == DMSM_Get_Master())
      {/*fout=fopen("sqrtrootsumall.dat","w");
       ...
       fprintf(fout, " Done with example3. \n");
       fclose(fout);

       printf(" Done with example3. \n");
      }
}
