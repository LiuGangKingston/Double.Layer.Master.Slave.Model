...
!    performed by OpenMP threads of a given node
!    in parallel, by employing special case 3
!    of the DMSM library.
!
...
void Do_The_Job(int my_job)
{
   ...
   r=(double)0.0;
#pragma omp parallel for default(shared) private(i) reduction(+:r)
...
{  FILE * fout;
   ...
   if (My_MPI_Rank == DMSM_Get_Master())
      {/*fout=fopen("sqrtrootsumall.dat","w");
       ...
       fprintf(fout, " Done by dmsm.example.job.openmp.c. \n");
       fclose(fout);

       printf(" Done by dmsm.example.job.openmp.c. \n");
      }
}
